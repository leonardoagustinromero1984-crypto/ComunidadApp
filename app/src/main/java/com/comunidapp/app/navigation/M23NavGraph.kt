...
fun NavGraphBuilder.m23BookingRoutes(navController: NavHostController) {
    ...
    composable(NavRoutes.M23_BOOKING_DETAIL, arguments = listOf(navArgument(NavRoutes.ARG_M23_BOOKING_ID) { type = NavType.StringType })) { entry ->
        val id = entry.arguments?.getString(NavRoutes.ARG_M23_BOOKING_ID).orEmpty()
        M23BookingDetailScreen(
            onBack = { navController.popBackStack() },
            viewModel = viewModel(factory = M23ViewModelFactories.detail(id))
        )
    }
