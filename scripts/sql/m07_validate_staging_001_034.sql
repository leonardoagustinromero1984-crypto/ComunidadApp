...
-- Alcance: migraciones 001–034 (historial, catálogos, RLS, grants, DEFINER, 033–034)
...
-- Contratos: org_hash_invitation_token = INVOKER + extensions.digest;
--            _resolve_invitation_by_token sin EXECUTE para anon/authenticated.
-- =============================================================================
...
-- ---------------------------------------------------------------------------
-- PASO 3 — Historial de migraciones 001–034
-- ---------------------------------------------------------------------------
do $$
declare
  v_count int;
...
begin
  ...
  perform pg_temp.m07_pass_fail(
    'HISTORY', 'migration_count', '34', v_count::text, v_count = 34,
    'versiones registradas en supabase_migrations.schema_migrations'
  );
  perform pg_temp.m07_pass_fail(
    'HISTORY', 'migration_max_version', '034', v_max, v_max = '034',
  ...
  perform pg_temp.m07_pass_fail(
    'HISTORY', 'migration_missing_001_034', '0', v_missing::text, v_missing = 0,
    format('faltantes entre 001–034: %s', v_missing)
  );
  perform pg_temp.m07_pass_fail(
    'HISTORY', 'migration_summary', '34 / 034 / 0 dup / 0 miss',
    format('%s / %s / %s dup / %s miss', v_count, v_max, v_dupes, v_missing),
    v_count = 34 and v_max = '034' and v_dupes = 0 and v_missing = 0,
...
declare
  ...
  v_names text[] := array[
    ...
    'invite_organization_member','_resolve_invitation_by_token',
  ...
  -- org_hash_invitation_token excluido: contrato real = INVOKER sin search_path obligatorio
...
begin
  ...
  perform pg_temp.m07_pass_fail(
    ...
    'funciones sensibles inventariadas (sin org_hash helper)'
  );

  -- Contrato correcto: org_hash_invitation_token = INVOKER + digest calificado
  select p.prosecdef, pg_get_functiondef(p.oid)
    into v_hash_definer, v_hash_def
  from pg_proc p
  join pg_namespace n on n.oid = p.pronamespace
  where n.nspname = 'public' and p.proname = 'org_hash_invitation_token'
  limit 1;

  perform pg_temp.m07_pass_fail(
    'DEFINER', 'org_hash_invitation_token_security_invoker', 'SECURITY INVOKER',
    case when coalesce(v_hash_definer, true) then 'SECURITY DEFINER' else 'SECURITY INVOKER' end,
    coalesce(v_hash_definer, true) = false,
    'helper puro; no elevar privilegios'
  );
  perform pg_temp.m07_pass_fail(
    'DEFINER', 'org_hash_invitation_token_qualified_digest', 'extensions.digest; no bare digest',
    case
      when v_hash_def like '%extensions.digest%'
       and replace(v_hash_def, 'extensions.digest', 'X') not like '%digest(%'
        then 'qualified'
      else 'unexpected'
    end,
    v_hash_def like '%extensions.digest%'
      and replace(v_hash_def, 'extensions.digest', 'X') not like '%digest(%',
    ''
  );
  perform pg_temp.m07_pass_fail(
    'DEFINER', 'org_hash_invitation_token_no_table_access', 'solo hash SQL',
    case
      when v_hash_def ~* '\b(from|insert|update|delete|into)\b' then 'table-like tokens'
      else 'hash-only shape'
    end,
    v_hash_def !~* '\b(from|insert|update|delete|into)\b',
    'estructural sobre pg_get_functiondef'
  );
  -- Contrato real: helper IMMUTABLE/INVOKER; EXECUTE a service_role suficiente;
  -- no exige REVOKE anon (no es superficie privilegiada).
  perform pg_temp.m07_pass_fail(
    'DEFINER', 'org_hash_invitation_token_grants',
    'service_role EXECUTE; no DEFINER surface',
    case
      when has_function_privilege('service_role', 'public.org_hash_invitation_token(text)', 'EXECUTE')
        then 'service_role=true'
      else 'service_role=false'
    end,
    has_function_privilege('service_role', 'public.org_hash_invitation_token(text)', 'EXECUTE'),
    'INVOKER puro; grants cliente no elevan privilegios'
  );

  -- 034: grants de _resolve_invitation_by_token
  select p.oid into v_resolve_oid
  from pg_proc p
  join pg_namespace n on n.oid = p.pronamespace
  where n.nspname = 'public'
    and p.proname = '_resolve_invitation_by_token'
  ...
  select
    ...
    exists (
      select 1 from aclexplode(coalesce(p.proacl, acldefault('f', p.proowner))) a
      join pg_roles rol on rol.oid = a.grantee
      where rol.rolname = 'anon' and a.privilege_type = 'EXECUTE'
    ),
    has_function_privilege('anon', p.oid, 'EXECUTE'),
    has_function_privilege('authenticated', p.oid, 'EXECUTE'),
    has_function_privilege('service_role', p.oid, 'EXECUTE')
  into v_pub, v_anon_d, v_anon_e, v_auth_e, v_svc_e
  from pg_proc p where p.oid = v_resolve_oid;

  perform pg_temp.m07_pass_fail('DEFINER', 'resolve_invitation_public_execute', 'false', coalesce(v_pub::text,'null'), coalesce(v_pub,true)=false, '034');
  perform pg_temp.m07_pass_fail('DEFINER', 'resolve_invitation_anon_execute_direct', 'false', coalesce(v_anon_d::text,'null'), coalesce(v_anon_d,true)=false, '034');
  perform pg_temp.m07_pass_fail('DEFINER', 'resolve_invitation_anon_execute_effective', 'false', coalesce(v_anon_e::text,'null'), coalesce(v_anon_e,true)=false, '034');
  perform pg_temp.m07_pass_fail('DEFINER', 'resolve_invitation_authenticated_execute', 'false', coalesce(v_auth_e::text,'null'), coalesce(v_auth_e,true)=false, '034');
  perform pg_temp.m07_pass_fail('DEFINER', 'resolve_invitation_service_role_execute', 'true', coalesce(v_svc_e::text,'null'), coalesce(v_svc_e,false)=true, '034');
